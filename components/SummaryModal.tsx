...
const SummaryModal: React.FC<SummaryModalProps> = ({ session, subject, onClose, onReset }) => {
  ...
  const downloadTxt = () => {
    ...
    // 加入 BOM (\uFEFF) 以確保 Excel 或 Windows 記事本能正確顯示繁體中文
    const blob = new Blob(['\uFEFF' + report], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    const dateStr = new Date().toISOString().split('T')[0];
    link.href = url;
    link.download = `Chronos觀課報告_${subject}_${dateStr}.txt`;
    link.click();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="glass-panel w-full max-w-4xl max-h-[90vh] rounded-3xl overflow-hidden flex flex-col z-10 border border-amber-500/30 shadow-2xl shadow-black">
        ...
        <div className="p-6 border-t border-amber-500/20 bg-slate-900/50 flex flex-wrap gap-4 justify-center md:justify-end">
          <button 
            onClick={downloadTxt}
            className="px-8 py-3 rounded-xl klimt-gradient text-slate-950 font-extrabold shadow-lg hover:brightness-110 active:scale-95 transition-all flex items-center gap-3"
          >
            <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
            下載觀課紀錄 (.txt)
          </button>
          <button 
            onClick={onReset}
            className="px-6 py-3 rounded-xl bg-red-800 text-white font-bold hover:bg-red-700 transition-colors flex items-center gap-2"
          >
            重置並開始新場次
          </button>
        </div>
